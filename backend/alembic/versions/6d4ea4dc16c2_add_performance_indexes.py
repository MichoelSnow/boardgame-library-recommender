...
def upgrade() -> None:
    ...
    op.create_index('ix_mechanics_boardgamemechanic_id', 'mechanics', ['boardgamemechanic_id'])
    op.create_index('ix_categories_game_id', 'categories', ['game_id'])
    op.create_index('ix_categories_boardgamecategory_id', 'categories', ['boardgamecategory_id'])
    op.create_index('ix_designers_game_id', 'designers', ['game_id'])
    op.create_index('ix_designers_boardgamedesigner_id', 'designers', ['boardgamedesigner_id'])
    ...
    op.create_index('ix_suggested_players_player_count', 'suggested_players', ['player_count'])
    op.create_index('ix_suggested_players_recommendation_level', 'suggested_players', ['recommendation_level'])
    
    # PAX games index for pax_only filtering
    op.create_index('ix_pax_games_bgg_id', 'pax_games', ['bgg_id'])
    
    # Composite indexes for common filter combinations
    op.create_index('ix_suggested_players_composite', 'suggested_players', ['game_id', 'player_count', 'recommendation_level'])
    op.create_index('ix_mechanics_composite', 'mechanics', ['game_id', 'boardgamemechanic_id'])
    op.create_index('ix_categories_composite', 'categories', ['game_id', 'boardgamecategory_id'])
