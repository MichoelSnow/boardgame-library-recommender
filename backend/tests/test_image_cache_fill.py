...
def test_cached_image_falls_back_to_proxy_for_non_fly_backend(monkeypatch):
    ...
    assert (
        ...
        == "/api/proxy-image/https%3A%2F%2Fexample.com%2Fgame.jpg"
    )
...
def test_cached_image_fly_local_downloads_and_redirects(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        main, "download_origin_image_content", mock_download_origin_image_content
    )
...
def test_cached_image_falls_back_to_db_lookup_when_image_url_missing(monkeypatch):
    ...
    monkeypatch.setattr(
        main.crud,
        "get_game",
        lambda db, game_id: SimpleNamespace(
            id=game_id, image="https://example.com/db-game.jpg"
        ),
    )
    ...
    assert (
        response.headers["location"]
        == "/api/proxy-image/https%3A%2F%2Fexample.com%2Fdb-game.jpg"
    )
