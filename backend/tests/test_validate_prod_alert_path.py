...
SCRIPT_PATH = REPO_ROOT / "scripts" / "validate_prod_alert_path.py"
...
def test_static_constants_match_expected_values() -> None:
    ...
    assert MODULE.EXPECTED_RUN_COMMAND == "python scripts/run_prod_health_alerts.py --env prod"


def test_paths_point_to_expected_files() -> None:
    assert MODULE.WORKFLOW_PATH.as_posix() == ".github/workflows/prod-health-alerts.yml"
    assert MODULE.ALERT_SCRIPT_PATH.as_posix() == "scripts/run_prod_health_alerts.py"
