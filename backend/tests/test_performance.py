...
async def test_endpoint(
    ...
    if times:
        ...
        print(
            f"  Results: Avg={avg_time:.1f}ms, Median={median_time:.1f}ms, Min={min_time:.1f}ms, Max={max_time:.1f}ms"
        )
        return {
            "name": name,
            "url": url,
...
async def main():
    ...
    for result in results:
        print(
            f"{result['name']:25} | Avg: {result['avg_time']:6.1f}ms | Median: {result['median_time']:6.1f}ms"
        )
