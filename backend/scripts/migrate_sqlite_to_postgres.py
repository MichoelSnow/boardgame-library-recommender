...
def fetch_source_rows(connection: Connection, table: Table) -> Iterator[dict]:
...
def migrate_sqlite_to_postgres(
    ...
    if anomaly_counts:
        logger.warning(
            "Source-data anomalies were normalized during migration: %s",
            ", ".join(
                f"{name}={count}" for name, count in sorted(anomaly_counts.items())
            ),
        )
