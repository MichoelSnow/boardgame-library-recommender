...
class BoardGame(Base):
    ...
    average_weight = Column(
        Float, index=True
    )  # average_weight - indexed for weight filtering
    stddev = Column(Float)
    median = Column(Float)
    ...
    suggested_players = relationship(
        "SuggestedPlayer", back_populates="game", lazy="select"
    )
    language_dependence = relationship(
        "LanguageDependence", back_populates="game", uselist=False, lazy="select"
    )
    integrations = relationship("Integration", back_populates="game", lazy="select")
    implementations = relationship(
        "Implementation", back_populates="game", lazy="select"
    )
...
class SuggestedPlayer(Base):
    ...
    recommendation_level = Column(
        String, index=True
    )  # 'best', 'recommended', or 'not_recommended' - indexed for filtering
...
Index(
    ...
    SuggestedPlayer.game_id,
    SuggestedPlayer.player_count,
    SuggestedPlayer.recommendation_level,
)
