...
## Naming Decision: `crawler/`
- Keep the directory name `crawler/`.
- Rationale:
  - existing code paths and team vocabulary already use `crawler`.
  - no ambiguity with runtime backend/frontend domains.
  - rename cost currently outweighs clarity gain.

## Location Boundaries
- Runtime/import logic for the backend stays under `backend/app/`.
- Pipeline execution code stays under `crawler/src/`.
- Exploratory analysis stays under `crawler/notebooks/`.
- Test utilities remain under `backend/tests/` and `crawler/tests/`; production scripts remain under `scripts/`.
...
## Notebook Policy
- Notebooks live only in `crawler/notebooks/`.
- Secrets/tokens/credentials must never be stored in notebooks.
- Notebook output should not contain secret-bearing payloads.
- Notebook logic that becomes production workflow must be migrated into `crawler/src/` modules/scripts.
...
- The canonical inventory is maintained in `crawler/README.md`.
