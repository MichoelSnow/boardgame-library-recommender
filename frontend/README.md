...
- Image delivery supports staged migration via optional environment variables:
  ...
  - `REACT_APP_IMAGE_CDN_BASE_URL`: optional backup CDN base URL (R2 fallback path), e.g. `https://cdn.example.com`
  - `REACT_APP_IMAGE_USE_PROXY_FALLBACK`: `true|false` (default `true`) to keep or disable backend proxy fallback safety net
  - `REACT_APP_IMAGE_PLACEHOLDER`: placeholder image path (default `/assets/images/game-placeholder.svg`)
  - Card images prefer thumbnails under `<base>/thumbnails/<bgg_id>.webp`; details prefer originals under `<base>/games/<bgg_id>.<ext>`.
